 ...
 */
import { CanvasLayer, Coordinate, Eventable, Util, renderer, ui } from 'maptalks';
import { Cartesian2, Cartesian3, Cartographic, Cesium3DTileset, Color, Ellipsoid, Globe, JulianDate, Primitive, Scene } from 'cesium';
...
var Mixins = {
    ...
    setToolTip: function setToolTip(content, options) {
        this.toolTip = new ui.ToolTip(content, options);
        return this;
    },
...
Util.extend(Primitive$1.prototype, Mixins);
...
}(Eventable(Cesium3DTileset));

Util.extend(Cesium3DTileset$1.prototype, Mixins);
...
var CesiumLayer = function (_maptalks$CanvasLayer) {
    ...
    */
    ...
    CesiumLayer.prototype.identify = function identify(coordinate, options) {
        ...
        var primitives = scene.pick(new Cartesian2(x, y));
        if (!primitives) {
            return [];
        }
        ...
        return Util.isNumber(count) && count > 0 ? primitives.slice(0, count) : primitives;
    };
...
var globalColor = new Color(0.0, 0.0, 0.0, 0.0);
...
export { CesiumLayer, Primitive$1 as Primitive, Cesium3DTileset$1 as Cesium3DTileset };
