...
class layerManager {
   ...
   */
  getClusterLayer(layerid, maxVisiableZoom = 7) {
    if (!this.Map) return;
    let layer = this.Map.getLayer(layerid);
    !layer && (
      layer = new markercluster.ClusterLayer(
        layerid,
        getClusterSymbol(maxVisiableZoom)
      ).addTo(this.Map)
    )
    return layer;
  }
   ...
   * @returns layer
   */
  getHeatMapLayer(layerid, coords) {
    let layer = new HeatLayer(layerid, coords, {
      heatValueScale: 100,
      forceRenderOnRotating: true,
      forceRenderOnMoving: true,
      max: 1,
   ...
   * {
    ...
    * @param {Number} time  动画时长(1000为一秒)
    * @param {function} call  动画结束回调
    */
