...
float noise (in vec3 p) {
	...
	f.xy = texture2D (iChannel0, (f.xy + .5) / 256., -256.).yx;
...
float windows (vec2 uv, float offset)
{
    ...
    float n2 = fbm((vec2(ivec2(uv * lightGrid + floor(time * .4) * .2)) + .5).xyx);
...
vec3 fireworks(vec2 st)
{
    ...
    for (int j = 0; j < FIREWORK_COUNT; ++j)
    {
        ...
        fireworkHash = hash31(471.5277 * float(j) + 1226.9146
			+ float(int((time+timeOffset) / FIREWORK_DURATION))) * 2. - 1.;
        ...
        float time = mod(time + timeOffset, FIREWORK_DURATION);
        if (time > ROCKET_DURATION)
        {
            ...
            for (int i = 0; i < EXPLOSION_PARTICLES; ++i)
            {
                ...
                shimmer = max(0., sqrt(sparkDistFromOrigin) 
                    * (sin((time + particleHash.y * TAU) * 18.)));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if (uv.y < 0.)
    {
        ...
        /**
        uv.x += sin((uv.y * 64. + sin(time) * .2)
			* cos(uv.y * 128. - cos(time) * .1)) * .15;
		*/
        
        // slightly better looking water waves
        uv.x += cos(uv.y * 192. - time * .6) * sin(uv.y * 96. + time * .75) * .042;
    }
