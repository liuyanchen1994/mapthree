...
vec2 polarMap(vec2 uv, float shift, float inner) {
    ...
    float py = (sqrt(uv.x * uv.x + uv.y * uv.y) * (1.0 + inner * 2.0) - inner/1.2) * 2.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec4 c1 = color(shade(uv, t)) * ff;
    vec4 c2 = color(shade(uv2, t)) * (1.0 - ff);
    
    vec3 color1=vec3(0.,0.,0.);
    vec4 color2=vec4(color1, 0.01);
    fragColor = color2 + c1 + c2;
}
void main(){
