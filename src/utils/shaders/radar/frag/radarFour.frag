...
const float r_blue = 0.015;		// size of blue radar blips
const float r_red = 0.005;		// size of red radar blips
const float edge = 0.98;		// overall size
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec4 mapcol = texture2D(iChannel0,uv) * vec4 (0.0, 0.85, 0.0, 1.0);
...
// Lets add all the bits together and send them to screen
    ...
    // Output to screen   
    fragColor = vec4(color, .2);//Set the screen pixel to that color
