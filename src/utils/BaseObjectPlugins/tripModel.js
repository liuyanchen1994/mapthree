...
class tripModel extends BaseObject {
  constructor(lineString, options, model, layer) {
    ...
    const { altitude, chunkLength, tempRing } = options;
    //generate geometry
    ...
    if (this.ring) {
      this.ring.getObject3d().position.x = position.x;
      this.ring.getObject3d().position.y = position.y;
    }
  ...
  _animation() {
    const { play, trip, rotation, stype } = this.getOptions();
    if (!play) {
      this.getMap().config("draggable", true);
      return;
    }
    ...
    // this.ring && this.ring.getObject3d().position.copy(_positions[this._idx]);

    if (this.ring) {
      this.ring.getObject3d().position.x = _positions[this._idx].x;
      this.ring.getObject3d().position.y = _positions[this._idx].y;
    }
    ...
    if (bearing != undefined) {
      ...
      //修改
      if (stype) this.model.rotation.y = bearing;
      else this.model.rotation.y = bearing + THREE.Math.degToRad(180);
      ...
      if (rotation) {
        ...
        if (Math.abs(bearing) >= Math.PI / 3) {
          const view = Object.assign({}, this.getMap().getView(), {
